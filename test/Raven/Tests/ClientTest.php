...
class Raven_Tests_ClientTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $result = Client::parseDsn('http://public:secret@example.com/1');
    ...
    {
        $result = Client::parseDsn('https://public:secret@example.com/1');
    ...
    {
        $result = Client::parseDsn('http://public:secret@example.com/app/1');
    ...
    {
        $result = Client::parseDsn('http://public:secret@example.com:9000/app/1');
    ...
    {
        try {
            $result = Client::parseDsn('gopher://public:secret@/1');
            $this->fail();
        } catch (Exception $e) {
            return;
        }
    ...
    {
        try {
            $result = Client::parseDsn('http://public:secret@/1');
            $this->fail();
        } catch (Exception $e) {
            return;
        }
    ...
    {
        try {
            $result = Client::parseDsn('http://public:secret@example.com');
            $this->fail();
        } catch (Exception $e) {
            return;
        }
    ...
    {
        $result = Client::parseDsn('http://:secret@example.com/1');
    }
    /**
     * @expectedException \InvalidArgumentException
     */
    ...
    {
        $result = Client::parseDsn('http://public@example.com/1');
    }
    ...
    {
        $client = new Client('http://public:secret@example.com/1', array(
            'site' => 'foo',
        ));
    ...
    public function testCaptureExceptionSetsInterfaces()
    {
        ...
        $this->assertEquals($frame['lineno'], 203);
    ...
    {
        $client = new Dummy_Client(array(
            'processors' => array('Raven\Processor'),
        ));
        $this->assertEquals(count($client->processors), 1);
        $this->assertTrue($client->processors[0] instanceof Processor);
    }
