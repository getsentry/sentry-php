...
class Raven_Tests_CompatTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $this->assertEquals(\Raven\Compat::gethostname(), \Raven\Compat::_gethostname());
        $this->assertTrue(strlen(\Raven\Compat::_gethostname()) > 0);
    }

    public function test_hash_hmac()
    {
        $result = \Raven\Compat::hash_hmac('sha1', 'foo', 'bar');
        $this->assertEquals('85d155c55ed286a300bd1cf124de08d87e914f3a', $result);

        $result = \Raven\Compat::_hash_hmac('sha1', 'foo', 'bar');
        $this->assertEquals('85d155c55ed286a300bd1cf124de08d87e914f3a', $result);
        ...
        $result = \Raven\Compat::_hash_hmac('md5', 'foobar', $long_key);
        $this->assertEquals('5490f3cddeb9665bce3239cbc4c15e2c', $result);

        $result = \Raven\Compat::_hash_hmac('sha1', 'foobar', $long_key);
        $this->assertEquals('5729f50ff2fbb8f8bf81d7a86f69a89f7574697c', $result);


        $result = \Raven\Compat::_hash_hmac('md5', 'foo', $long_key);
        $this->assertEquals('ab193328035cbd3a48dea9d64ba92736', $result);

        $result = \Raven\Compat::_hash_hmac('sha1', 'foo', $long_key);
        $this->assertEquals('8f883d0755115314930968496573f27735eb0c41', $result);
    }

    public function test_json_encode()
    {
        $result = \Raven\Compat::json_encode(array('foo' => array('bar' => 1)));
        $this->assertEquals('{"foo":{"bar":1}}', $result);

        $result = \Raven\Compat::_json_encode(array('foo' => array('bar' => 1)));
        $this->assertEquals('{"foo":{"bar":1}}', $result);

        $result = \Raven\Compat::_json_encode(array(1, 2, 3, 4, 'foo', 'bar'));
        $this->assertEquals('[1,2,3,4,"foo","bar"]', $result);

        $result = \Raven\Compat::_json_encode(array(1, 'foo', 'foobar' => 'bar'));
        $this->assertEquals('{0:1,1:"foo","foobar":"bar"}', $result);

        $result = \Raven\Compat::_json_encode(array(array()));
        $this->assertEquals('[[]]', $result);

        $result = \Raven\Compat::_json_encode(array(null, false, true, 1.5));
    ...
    public function test_json_encode_with_broken_data()
    {
        ...
        $value_1024 = \Raven\Compat::_json_encode($data_broken);
        $value_510 = \Raven\Compat::_json_encode($data_broken_510);
        $value_511 = \Raven\Compat::_json_encode($data_broken_511);
        ...
        $value_1024 = \Raven\Compat::_json_encode($data_broken_named);
        $value_510 = \Raven\Compat::_json_encode($data_broken_named_510);
        $value_511 = \Raven\Compat::_json_encode($data_broken_named_511);
        $this->assertFalse($value_1024, 'Broken data encoded successfully with Raven_Compat::_json_encode');
        $this->assertNotFalse($value_510);
        $this->assertFalse($value_511);
    }
